...
<a wire:navigate href="{{ route('careers.show', $career) }}"
    ...
    <div class="absolute inset-0 z-0 transition-transform duration-700">
        ...
        <div
            class="absolute inset-0 opacity-10 bg-[radial-gradient(#ffffff33_1px,transparent_1px)] [background-size:16px_16px]">
        </div>
    </div>
    ...
    <div class="relative z-10">
        <div class="flex items-start justify-between mb-4">
            ...
            @if($career->is_active)
                <span
                    class="px-2 py-1 text-[8px] font-black uppercase tracking-wider bg-brand-yellow/20 text-brand-yellow rounded border border-brand-yellow/30">Active</span>
            @else
                <span
                    class="px-2 py-1 text-[8px] font-black uppercase tracking-wider bg-white/5 text-white/30 rounded border border-white/10">Closed</span>
            @endif
        </div>
    ...
    <div class="relative z-10 flex items-center justify-between mt-8">
        <span
            class="text-[9px] font-black uppercase tracking-[0.2em] text-brand-yellow group-hover:translate-x-2 transition-transform">Apply
            Now —</span>
