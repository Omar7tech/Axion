    ...
    <div class="container mx-auto px-6 lg:px-20 max-w-7xl relative z-10">
        ...
        @if($items->count() > 0)
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
                @foreach($items as $index => $item)
                    <div wire:key="why-us-{{ $item->id }}" class="group relative bg-white/[0.02] border border-white/5 p-8 transition-all duration-500 hover:bg-white/[0.04] hover:border-brand-yellow/30 flex flex-col justify-between min-h-[280px] md:min-h-[320px] backdrop-blur-sm">
        ...
        @else
            <div class="text-center py-16">
                <p class="text-white/40 text-lg uppercase tracking-widest">No items available</p>
            </div>
        @endif
    </div>
</section>
