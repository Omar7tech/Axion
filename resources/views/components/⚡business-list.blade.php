        ...
        <a @if($business->link) target="_blank" @endif
            ...
            <div class="relative z-10">
                ...
                <p class="text-[10px] text-white/90 font-bold uppercase tracking-tight mb-3 italic leading-tight">
                    {{ $business->subtitle }}</p>
                <p class="text-[11px] text-white/50 leading-relaxed max-w-[90%]">{{ Str::limit($business->description, 100) }}</p>
            </div>

            <div class="relative z-10 flex items-center justify-between mt-8">
                <span class="text-[9px] font-black uppercase tracking-[0.2em] text-brand-yellow group-hover:translate-x-2 transition-transform">{{ $business->link ? 'Visit' : 'Show more' }} —</span>
